...
class Game {
  ...
  addPlatforms() {
    ...
    this.platforms.push(new Platforms(600,250,150,8));
    this.platforms.push(new Platforms(800,150,100,8));
    this.platforms.push(new Platforms(1100,200,250,8));
  ...
  checkCollisions(player, object) {
    ...
    if (playerX < object.x + object.width &&
      ...
      player.height + playerY > object.y) {
       let y = (playerY + player.height/2) - (object.y + object.height);
       let x = (playerX + player.width) - (object.x + object.width);
  ...
  createPlatforms(i) {
    let offset = 1024;
    if(this.speed > 5) {
      offset += (offset%this.speed * this.speed);
    }
  ...
  gameOver(dir = "") {
    if(this.player.pos[1] > Game.DIM_Y - 100) {
      this.player.key = 2;
      if(this.player.pos[1] >= Game.DIM_Y-90) {
        return true;
      }
    }
